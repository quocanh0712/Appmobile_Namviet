// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target

part of 'schedule_daily_request.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

ScheduleDailyRequest _$ScheduleDailyRequestFromJson(Map<String, dynamic> json) {
  return _ScheduleDailyRequest.fromJson(json);
}

/// @nodoc
mixin _$ScheduleDailyRequest {
  String? get nowdate => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $ScheduleDailyRequestCopyWith<ScheduleDailyRequest> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $ScheduleDailyRequestCopyWith<$Res> {
  factory $ScheduleDailyRequestCopyWith(ScheduleDailyRequest value,
          $Res Function(ScheduleDailyRequest) then) =
      _$ScheduleDailyRequestCopyWithImpl<$Res, ScheduleDailyRequest>;
  @useResult
  $Res call({String? nowdate});
}

/// @nodoc
class _$ScheduleDailyRequestCopyWithImpl<$Res,
        $Val extends ScheduleDailyRequest>
    implements $ScheduleDailyRequestCopyWith<$Res> {
  _$ScheduleDailyRequestCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? nowdate = freezed,
  }) {
    return _then(_value.copyWith(
      nowdate: freezed == nowdate
          ? _value.nowdate
          : nowdate // ignore: cast_nullable_to_non_nullable
              as String?,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$_ScheduleDailyRequestCopyWith<$Res>
    implements $ScheduleDailyRequestCopyWith<$Res> {
  factory _$$_ScheduleDailyRequestCopyWith(_$_ScheduleDailyRequest value,
          $Res Function(_$_ScheduleDailyRequest) then) =
      __$$_ScheduleDailyRequestCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call({String? nowdate});
}

/// @nodoc
class __$$_ScheduleDailyRequestCopyWithImpl<$Res>
    extends _$ScheduleDailyRequestCopyWithImpl<$Res, _$_ScheduleDailyRequest>
    implements _$$_ScheduleDailyRequestCopyWith<$Res> {
  __$$_ScheduleDailyRequestCopyWithImpl(_$_ScheduleDailyRequest _value,
      $Res Function(_$_ScheduleDailyRequest) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? nowdate = freezed,
  }) {
    return _then(_$_ScheduleDailyRequest(
      nowdate: freezed == nowdate
          ? _value.nowdate
          : nowdate // ignore: cast_nullable_to_non_nullable
              as String?,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_ScheduleDailyRequest implements _ScheduleDailyRequest {
  const _$_ScheduleDailyRequest({this.nowdate});

  factory _$_ScheduleDailyRequest.fromJson(Map<String, dynamic> json) =>
      _$$_ScheduleDailyRequestFromJson(json);

  @override
  final String? nowdate;

  @override
  String toString() {
    return 'ScheduleDailyRequest(nowdate: $nowdate)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_ScheduleDailyRequest &&
            (identical(other.nowdate, nowdate) || other.nowdate == nowdate));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(runtimeType, nowdate);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_ScheduleDailyRequestCopyWith<_$_ScheduleDailyRequest> get copyWith =>
      __$$_ScheduleDailyRequestCopyWithImpl<_$_ScheduleDailyRequest>(
          this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_ScheduleDailyRequestToJson(
      this,
    );
  }
}

abstract class _ScheduleDailyRequest implements ScheduleDailyRequest {
  const factory _ScheduleDailyRequest({final String? nowdate}) =
      _$_ScheduleDailyRequest;

  factory _ScheduleDailyRequest.fromJson(Map<String, dynamic> json) =
      _$_ScheduleDailyRequest.fromJson;

  @override
  String? get nowdate;
  @override
  @JsonKey(ignore: true)
  _$$_ScheduleDailyRequestCopyWith<_$_ScheduleDailyRequest> get copyWith =>
      throw _privateConstructorUsedError;
}
