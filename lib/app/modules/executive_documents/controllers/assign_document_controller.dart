...
class AssignDocumentController
    extends BaseListController<AssignDocumentResponse> {
  @override
  void onInit() async {
    Iterable list = json
        .decode(await rootBundle.loadString(Assets.jsons.assignDocumentList));
    listData.value = List<AssignDocumentResponse>.from(
        list.map((model) => AssignDocumentResponse.fromJson(model)));
    super.onInit();
  }
  ...
  void updateAssignDocument(int index) async {
    Get.toNamed(Routes.EDIT_ASSIGN_DOCUMENT);
  }

  //void removeAssignDocument(int index) async {}

  @override
  Future<Result<BaseResponseObject<List<AssignDocumentResponse?>?>, Exception>>
      callToHost() {
