...
class LearningProcessController extends BaseListController<LearningProcessResponse> {
  @override
  void onInit() {
    super.onInit();
  }
  ...
  @override
  Future<Result<BaseResponseObject<List<LearningProcessResponse?>?>, Exception>> callToHost() {
    WorkingProcessRepository repository = Get.find();
    return repository.getLearningProcessResponseList(
        WorkingProcessRequest(length: maxLengthResult, startindex: pageIndex));
  }

  void createNewWorkingProcess() async {
    var resultCreate = await Get.toNamed(Routes.CREATE_LEARNING_PROCESS, arguments: [true, null]);
  ...
  void updateWorkingProcess(int index) async {
    var resultCreate =
        await Get.toNamed(Routes.CREATE_LEARNING_PROCESS, arguments: [false, getItem(index)]);
    if (resultCreate is bool && resultCreate) {
      refreshData();
    }
  }
  ...
  removeWorkingProcess(int index) async {
    ...
    var response = await repo.deleteLearningProcess(getItem(index));
